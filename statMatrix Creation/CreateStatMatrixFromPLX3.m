function CreateStatMatrixFromPLX3(plxFile)
%% CreateStatMatrixFromPLX3
...
% on.
%
% v3 created with modified channel selection protocols to ensure LFP and
% unit channel names are matching correctly.
...
for tet = 1:size(plxADchanNames,1)
    ...
    tetName = deblank(curADchan);
    tetName(end-1:end) = [];
    
    fprintf('Found LFP data for %s\n', tetName);
    [samp, ~, tetTS, tetFN, tetV] = plx_ad_v(plxFile, curADchan);
    ...
    curChanNums = find(strcmp(tetName,tetNames));
    curNumUnis = numUnis(curChanNums(1));
    ...
    for uni = 1:curNumUnis
        ...
        [~, tempTSs] = plx_ts(plxFile, curChanNums(1), uni);
        tetUnitTSs{uni} = histcounts(tempTSs, tsVect)';
        wire = 1;
        for tetChan = 1:length(curChanNums)
            [~, ~, ~, wave] = plx_waves_v(plxFile, curChanNums(tetChan), uni);
