...
for f = 1:length(smFileList)
    ...
    betaCol = cellfun(@(a)~isempty(a), regexp(statMatrixColIDs, 'Beta\>'));
...
%     iscBetaRS = cell2mat(ExtractTrialData_SM(tdPO(isLog & perfLog), zEnv));
    ...
    for t = 1:size(iscBetaRS,1)
        tempCorrPD = corrcoef(iscBetaRS(t,:), pdTrls);
        pdCorr(t) = tempCorrPD(2);
        tempCorrWL = corrcoef(iscBetaRS(t,:), wlTrls);
        wlCorr(t) = tempCorrWL(2);
    end
    subplot(2,4,7:8);
    plot(poWindow(1):1/1000:poWindow(2), pdCorr, 'color', 'k');
    hold on; 
    plot(poWindow(1):1/1000:poWindow(2), wlCorr, 'color', 'k', 'linestyle', '-.');
    legend('Poke Duration', 'Withdrawal Latency', 'location', 'northwest');
